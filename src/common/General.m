...
- (NSString *) transactionGetCacheUrl : (NSString *) hash {
    if ([Tools isEmpty: hash]) {
        return [NSString stringWithFormat : @"%@/getTransactionCache", _url];
    } else {
        return [NSString stringWithFormat : @"%@/getTransactionCache?hash=%@", _url, hash];
    }
