...
/**
 Get the cache of a transaction
 @param transactionGetCacheRequest
            hash(NSString *) : The transaction hash
...
- (TransactionGetCacheResponse *) getCache : (TransactionGetCacheRequest *) transactionGetCacheRequest {
    TransactionGetCacheResponse *transactionGetCacheResponse = [TransactionGetCacheResponse new];
    TransactionGetCacheResult *transactionGetCacheResult = [TransactionGetCacheResult new];
    @try {
        if ([Tools isEmpty:transactionGetCacheRequest]) {
            @throw [[SDKException alloc] initWithCode : REQUEST_NULL_ERROR];
        }
        NSString *hash = [transactionGetCacheRequest getHash];
        if (![Tools isEmpty: hash] && [hash length] != HASH_HEX_LENGTH) {
            @throw [[SDKException alloc] initWithCode : INVALID_HASH_ERROR];
        }
        transactionGetCacheResponse = [TransactionServiceImpl getTransactionCache : hash];
        int32_t errorCode = transactionGetCacheResponse.errorCode;
        if (errorCode == 4) {
            @throw [[SDKException alloc] initWithCodeAndDesc : errorCode : [NSString stringWithFormat : @"Transaction (%@) doest not exist", hash]];
        }
    }
    @catch(SDKException *sdkException) {
        [transactionGetCacheResponse buildResponse: ([sdkException getErrorCode]) : [sdkException getErrorDesc] :(transactionGetCacheResult)];
    }
    @catch(NSException *exception) {
        [transactionGetCacheResponse buildResponse: (SYSTEM_ERROR) : [exception reason] :(transactionGetCacheResult)];
    }
    return transactionGetCacheResponse;
}
    ...
    for (BaseOperation *baseOperation in operations) {
        ...
        switch (operationType) {
            ...
            case ASSET_SEND:
                ...
                break;
            case GAS_SEND:
                operation = [GasServiceImpl send : (GasSendOperation *)baseOperation : sourceAddress];
                break;
            ...
            case CONTRACT_INVOKE_BY_GAS:
                operation = [ContractServiceImpl invokeByGas : (ContractInvokeByGasOperation *)baseOperation : sourceAddress];
                break;
            case LOG_CREATE:
    ...
    for (Operation *operation in transaction.operationsArray) {
        ...
        switch (type) {
            ...
            case Operation_Type_PayCoin: {
                GasSendInfo *buSendInfo = [GasSendInfo new];
                ...
                operationInfo.sendGas = buSendInfo;
            }
                break;
...
+ (TransactionGetCacheResponse *) getTransactionCache : (NSString *) hash {
    if ([Tools isEmpty : [[General sharedInstance] getUrl]]) {
        @throw [[SDKException alloc] initWithCode : URL_EMPTY_ERROR];
    }
    NSString *getCacheUrl = [[General sharedInstance] transactionGetCacheUrl: hash];
    NSData *result = [Http get: getCacheUrl];
    return [TransactionGetCacheResponse yy_modelWithJSON: result];
}
