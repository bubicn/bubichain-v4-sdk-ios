...
//    NSString *resourcePath = [bundle pathForResource:@"mnemonic.bundle/english" ofType:@"txt"];
...
//    NSArray *lines = [fileText componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]];
    NSArray *lines = [self englishWordList];
...
+ (NSArray *) generatePrivateKeys: (NSArray *)mnemonicCodes : (NSArray *)hdPaths {
    ...
    ChainKey *chainKey = [[ChainKey alloc] initWithSeed: seed];
    NSMutableArray *privateKeys = [NSMutableArray new];
    for (NSString *hdPath in hdPaths) {
        ChainKey *chainKeyIndex = [chainKey derivedKeychainWithPath:hdPath];
        NSString *privateKey = [chainKeyIndex getEncPrivateKey];
